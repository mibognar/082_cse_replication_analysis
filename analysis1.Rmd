...
cs_rt_plot = filtered_rt %>% 
  ...
  aes(x=factor(congruency, level=c("no_conflict","low_conflict", "high_conflict")), y=rt, fill=factor(congruency, level=c("no_conflict","low_conflict", "high_conflict")),show.legend=F)+
...
### Analyze with bayesian lmer
```{r}
main_cl_model = lmer(rt ~ congruency_helmert + (1|id), data = contrast_filtered_rt)
...
### Analyze with bayesian lmer
```{r}
main_cse_model = lmer(rt ~ congruency_helmert*previous_congruency_helmert + (1|id), data = contrast_filtered_rt)
